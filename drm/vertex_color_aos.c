...
static int colorbuffer_handle;
static int vertexbuffer_handle;
...
int indirect_buffer()
{
  ...
  // VAP_VTX_SIZE is only used for vertex data passed via IMMD
  //T0V(VAP_VTX_SIZE
  ...
  ib[ix++].u32 // VAP_VTX_AOS_ATTR01
    ...
    | VAP_VTX_AOS_ATTR__VTX_AOS_STRIDE0(6)
    | VAP_VTX_AOS_ATTR__VTX_AOS_COUNT1(3)
    | VAP_VTX_AOS_ATTR__VTX_AOS_STRIDE1(6)
    ;
...
int main()
{
  ...
  // vertex buffer
  {
    ...
    vertexbuffer_handle = args.handle;
  }

  {
    ...
    vertexbuffer_ptr = mmap(0, mmap_args.size, PROT_READ|PROT_WRITE, MAP_SHARED,
                            fd, mmap_args.addr_ptr);
  }
  // copy vertices
  {
    for (int i = 0; i < vertices_length; i++) {
      ((float*)vertexbuffer_ptr)[i] = vertices[i];
    }
    asm volatile ("" ::: "memory");
  }
