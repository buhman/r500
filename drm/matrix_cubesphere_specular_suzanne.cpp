...
const int triangle_count = suzanne_model.object[0]->triangle_count;
...
int _3d_cube_inner(int ix, mat4x4 trans, mat4x4 world_trans, vec4 light_pos, vec4 view_pos)
{
  ...
  T3(_NOP, 0);
  ib[ix++].u32 = 3 * 4; // index into relocs array for VAP_VTX_AOS_ADDR2
  //T3(_NOP, 0);
  //ib[ix++].u32 = 3 * 4; // index into relocs array for VAP_VTX_AOS_ADDR3
...
int _3d_cube(int ix, float theta)
{
  ...
  if (1) {
    ...
    ix = _3d_cube_inner(ix, trans, world_trans, light_pos, view_pos);
  }
...
void fill_vertexbuffer(void * vertexbuffer_ptr, int vertexbuffer_size)
{
  int ix = 0;

  float * vbuf = (float *)vertexbuffer_ptr;

  const model * model = &suzanne_model;
  ...
  for (int i = 0; i < triangle_count; i++) {
    for (int j = 0; j < 3; j++) {
      ...
      assert((ix + 8) * 4 < vertexbuffer_size);
...
int main()
{
  ...
  fill_vertexbuffer(vertexbuffer_ptr, vertexbuffer_size);
